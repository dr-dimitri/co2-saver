...
- **Zugehörige Issues:** [#1](https://github.com/dr-dimitri/co-saver/issues/1), [#17](https://github.com/dr-dimitri/co-saver/issues/17), [#20](https://github.com/dr-dimitri/co-saver/issues/20), [#26](https://github.com/dr-dimitri/co-saver/issues/26)
...
Version 2.2 entscheidet die CO₂-Probenwahl aus #26: Jeder reguläre UTC-Minutenpoll kopiert für die laufende Bilanzierung genau den aktuellen Netz-CO₂-State als unveränderliche Beobachtung. Nur diese Beobachtung darf das in diesem Poll vollständig verarbeitete Energieintervall bewerten. Es gibt weder zusätzliche State-Event-Reads noch eine Probenhistorie oder einen Rückgriff auf ältere Proben. Eine am physischen Intervallende unzulässige aktuelle Probe lässt die gutschriftfähige Energie dauerhaft unbewertet.
...
Für die laufende Bilanzierung kopiert der Adapter bei jedem regulären UTC-Minutenpoll mit `second = 0` genau einmal den aktuellen Netz-CO₂-State einschließlich Wert, Einheit, Registry-ID und `State.last_reported` als unveränderliche Beobachtung. Eine fehlende oder ungültige Beobachtung wird ebenfalls nur einmal festgestellt. `state_changed` und `state_reported` lösen keine zusätzlichen CO₂-Reads aus. Verpasste Polls werden nicht nachgeholt. Die reine Quellenvalidierung im Config Flow ist keine Laufzeitbeobachtung und befüllt keinen Probenpuffer.

Ein in diesem Poll vollständig verarbeitetes Energieintervall verwendet ausschließlich diese aktuelle CO₂-Probe. Ihr `observed_at` muss kleiner oder gleich dem physischen Intervallende aus `co2saver_period_end` sein; das Alter `Intervallende - observed_at` darf den konfigurierten Grenzwert nicht überschreiten. Sowohl Gleichheit mit dem Intervallende als auch exakt erreichtes Höchstalter sind zulässig. Die Poll-Uhrzeit ersetzt das physische Intervallende bei dieser Prüfung nicht. Für direkten PV-Verbrauch ist der Verbrauchszeitpunkt maßgeblich, für Speicherenergie der Entladezeitpunkt.

Wird ein zunächst unvollständiger Energiekandidat erst in einem späteren Poll vervollständigt, gilt ausschließlich die aktuelle CO₂-Probe dieses späteren Verarbeitungspolls, geprüft gegen das unveränderte physische Energieintervallende. Eine beim Beginn des Kandidaten gelesene CO₂-Probe wird weder mit dem Kandidaten aufbewahrt noch später verwendet.

Es gibt keinen flüchtigen oder persistenten CO₂-Probenverlauf und keinen Rückgriff auf eine zuvor beobachtete Probe. Das gilt gleichermaßen bei einem fehlenden, `unknown`, `unavailable`, nicht numerischen, inkompatiblen, veralteten oder für das Intervall zukünftigen aktuellen State. Auch eine erneute Meldung desselben Wertes wird ausschließlich mit ihrem aktuellen `last_reported` geprüft. Neustart, Reload und Segmentwechsel stellen keine alte CO₂-Probe wieder her; erst der nächste reguläre Minutenpoll liefert die aktuelle Beobachtung. Eine Netzintensität vom Ladezeitpunkt wird nicht gespeichert.

Beispiel aus #26: Beim Poll um `12:01` endet das Energieintervall physisch um `12:00:30`. Eine früher beobachtete Probe von `11:59` betrug `400 g CO₂e/kWh`; der aktuelle State meldet seit `12:00:45` einen Wert von `600 g CO₂e/kWh`. Auch bei 60 Minuten erlaubtem Höchstalter bleibt das Intervall ohne Emissionsbuchung, weil ausschließlich die aktuelle, für das Intervall zukünftige Probe geprüft wird. Die gutschriftfähige Energie wird als unbewertet erfasst. Die ältere Probe wird nicht verwendet und eine spätere gültige Probe führt zu keiner Nachbewertung.
...
- **Ungültige CO₂-Probe:** Ausschließlich die aktuelle CO₂-Beobachtung des Polls, in dem das vollständige Energieintervall verarbeitet wird, wird gemäß Abschnitt 4.3 geprüft. Eine fehlende, zukünftige, veraltete oder semantisch ungültige Beobachtung, insbesondere `unknown` oder `unavailable`, erlaubt keinen Rückgriff auf ältere Proben und unterbricht nicht den physischen Energie- und Herkunftsledger. Das Intervall erhält aber keinerlei Emissionsbuchung. Seine gutschriftfähige Energie wird in getrennten Diagnosezählern für unbewertete direkte und gespeicherte PV-Energie erfasst und nie später mit einem aktuellen Wert nachbewertet.
...
- **Home-Assistant-Neustart:** Auswertungszustand, Energie-Baselines, `recovery_after_period_end` und gepufferte Energiekandidaten mit deklarierten Einheiten, Werten, `co2saver_period_end` und `last_reported`, Segmentfingerabdruck, kumulative Emissionskomponenten, Diagnosezähler, `S_lo`, `S_hi`, `P_lo`, `N_hi`, `B_pv`, `rho_pv`, Kapazität und Schemaversion werden als eine atomare Generation persistiert und vor jeder neuen Auswertung wiederhergestellt. CO₂-Proben gehören nicht zu diesem Zustand. Neustart, Reload und Segmentwechsel verwenden auch bei einem wiederhergestellten Energiekandidaten ausschließlich die aktuelle CO₂-Beobachtung des nächsten tatsächlich auswertenden regulären Polls.
...
- jeder reguläre UTC-Minutenpoll kopiert genau einmal die aktuelle Netz-CO₂-Beobachtung unveränderlich; State-Events, Setup, Reload und verpasste Polls erzeugen keine zusätzlichen Laufzeit-Reads oder nachgeholten Beobachtungen;
- ausschließlich die aktuelle CO₂-Probe des Verarbeitungspolls ist bei `observed_at` exakt am physischen Energieintervallende und bei exakt erreichtem Maximalalter gültig; unmittelbar spätere beziehungsweise ältere Proben sowie `unknown`, `unavailable`, nicht numerische Werte und inkompatible Einheiten sind ungültig. Dann erfolgen keine Emissionsbuchungen, die gutschriftfähige Energie erhöht die getrennten Unbewertet-Zähler, und ein betroffener Speicherledger wird normal aktualisiert; es gibt weder einen Rückgriff auf frühere Proben noch eine spätere Nachbewertung;
- beim Poll `12:01` mit Energieintervallende `12:00:30`, früher beobachteter gültiger CO₂-Probe `11:59` und aktuellem State von `12:00:45` bleibt die Energie unbewertet. Dasselbe gilt bei aktuell `unknown` oder `unavailable`, unabhängig von einer früheren gültigen Probe;
- ein im nächsten Poll vervollständigter oder nach Neustart wiederhergestellter Energiekandidat verwendet ausschließlich die aktuelle CO₂-Beobachtung des tatsächlichen Verarbeitungspolls. CO₂-Beobachtungen vom Kandidatenbeginn, vor Reload oder vor einem Segmentwechsel werden nicht aufbewahrt oder wiederhergestellt;
...
| 2.1 | Ersetzt | Präzisierung aus #20: ausschließlich 60-sekündliches Polling für Energiezustände, konservative Rebaseline bei jedem Einheitenwechsel und klare Injektionsgrenze zwischen Messpipeline und Config-/Store-Bootstrap. |
| 2.2 | Gültig | Entscheidung aus #26: ausschließlich die unveränderlich kopierte aktuelle Netz-CO₂-Beobachtung des tatsächlichen Minuten-Verarbeitungspolls, gültig am physischen Energieintervallende; kein State-Event-Read, Probenverlauf, Rückgriff oder Wiederherstellen älterer CO₂-Proben. |
